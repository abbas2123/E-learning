...
router.post("/courses/:courseId/sections", authMiddleware, (req, res, next) =>
  controller.createSection(req, res, next),
);

// 3. PUT /api/sections/:sectionId (Auth required)
router.put("/sections/:sectionId", authMiddleware, (req, res, next) =>
  controller.updateSection(req, res, next),
);

// 4. DELETE /api/sections/:sectionId (Auth required)
router.delete("/sections/:sectionId", authMiddleware, (req, res, next) =>
...
router.post("/sections/:sectionId/lessons", authMiddleware, (req, res, next) =>
  controller.createLesson(req, res, next),
);

// 7. PUT /api/lessons/:lessonId (Auth required)
router.put("/lessons/:lessonId", authMiddleware, (req, res, next) =>
  controller.updateLesson(req, res, next),
);

// 8. DELETE /api/lessons/:lessonId (Auth required)
router.delete("/lessons/:lessonId", authMiddleware, (req, res, next) =>
