  ...
  async execute(input: MarkLessonCompleteInput): Promise<LessonProgressDto> {
    ...
    if (this.lessonRepository) {
      ...
      if (!lesson) {
        ...
        const quizLesson = await this.lessonRepository.findByQuizOrLessonId(courseId, lessonId);
        if (!quizLesson) throw new Error("Lesson or assessment not found.");
    ...
    const existingProgress = await this.progressRepository.findByLesson(userId, courseId, lessonId);
    const currentWatch =
      watchedSeconds !== undefined
        ? Math.min(durationSeconds, Math.max(0, watchedSeconds))
        : existingProgress?.watchedSeconds ?? (lessonType === "video" ? 0 : durationSeconds);
