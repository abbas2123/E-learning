...
import { VIDEO_COMPLETION_THRESHOLD, DEFAULT_MIN_CERTIFICATE_SCORE } from "../shared/constants/courseConstants.js";
...
function createMockCertificateRepo(existingCert: any = null) {
  ...
  return {
    ...
    verifyCertificate: async () => (cert ? { isValid: true, certificate: cert } : { isValid: false, message: "Not found" }),
...
function createMockLessonRepo(lessons: any[] = []) {
  return {
    ...
    findByQuizOrLessonId: async (courseId: string, id: string) => lessons.find((l) => l.id === id || l.quizId === id) || null,
...
function createMockProgressRepo(records: any[] = []) {
  ...
  return {
    findByLesson: async (studentId: string, courseId: string, lessonId: string) =>
      store.get(`${studentId}:${courseId}:${lessonId}`) || null,
    findByCourse: async (studentId: string, courseId: string) =>
      Array.from(store.values()).filter((r) => r.studentId === studentId && r.courseId === courseId),
    upsertProgress: async (dto: any) => {
      store.set(`${dto.studentId}:${dto.courseId}:${dto.lessonId}`, dto);
      return dto;
    },
    getCourseCompletionStats: async () => ({ completedLessons: 0, totalLessons: 0, percentage: 0 }),
    countCompletedLessons: async (studentId: string, courseId: string) =>
      Array.from(store.values()).filter((r) => r.studentId === studentId && r.courseId === courseId && r.completed).length,
  };
}
...
function createMockQuizRepo(quizzes: any[] = []) {
  return {
    ...
    findByLessonId: async (lId: string) => quizzes.find((q) => q.lessonId === lId) || null,
    update: async () => null,
    delete: async () => true,
  };
}
...
function createMockAttemptRepo(attempts: any[] = []) {
  return {
    ...
    findSubmittedByStudentAndQuiz: async (s: string, q: string) =>
      attempts.filter((a) => a.studentId === s && a.quizId === q && a.status === "submitted"),
    getBestPercentageByStudentAndQuiz: async (s: string, q: string) => {
      const matched = attempts.filter((a) => a.studentId === s && a.quizId === q && a.status === "submitted");
      return matched.length > 0 ? Math.max(...matched.map((m) => m.percentage || 0)) : 0;
    },
...
test("GetCertificateStatusUseCase: blocks ineligible student when lessons are incomplete", async () => {
  ...
  const progress = [{ studentId: "u_1", courseId: "c_1", lessonId: "les_1", completed: true, watchedSeconds: 600 }];
...
test("GetCertificateStatusUseCase: grants eligibility when all lessons and quizzes pass minimum score", async () => {
  const lessons = [{ id: "les_1", courseId: "c_1", duration: 10, type: "video" }];
  const quizzes = [{ id: "quiz_1", courseId: "c_1", passingScore: 70 }];
  const progress = [{ studentId: "u_1", courseId: "c_1", lessonId: "les_1", completed: true, watchedSeconds: 600 }];
  const attempts = [
    { studentId: "u_1", quizId: "quiz_1", status: "submitted", percentage: 85, passed: true },
  ];
...
test("UpdateLessonWatchProgressUseCase: correctly computes 90% threshold for video completion", async () => {
  const lessons = [{ id: "les_1", courseId: "c_1", duration: 10, type: "video" }]; // 600s total, threshold = 540s (90%)
...
test("GenerateCertificateUseCase: is idempotent and prevents unearned certificates", async () => {
  const lessons = [{ id: "les_1", courseId: "c_1", duration: 10, type: "video" }];
  const progress = [{ studentId: "u_1", courseId: "c_1", lessonId: "les_1", completed: false, watchedSeconds: 100 }];
...
test("ApplyInstructorUseCase: submits application and prevents duplicate for existing instructor", async () => {
  ...
  const useCaseInst = new ApplyInstructorUseCase(createMockUserRepo(existingInstructor), notifRepo);
  ...
  // 2. Student applicant
  const useCaseStudent = new ApplyInstructorUseCase(createMockUserRepo(studentUser), notifRepo);
  const res2 = await useCaseStudent.execute({ userId: "student_1", expertise: "Computer Science" });
...
test("GetUserNotificationsUseCase: retrieves user-scoped notifications and accurate unread count", async () => {
  const store = [
    { id: "n_1", title: "Payment Received", message: "Course unlocked", userId: "u_1", read: false, createdAt: new Date() },
    { id: "n_2", title: "System Update", message: "Maintenance scheduled", userId: null, read: true, createdAt: new Date() },
    { id: "n_3", title: "Certificate Ready", message: "View your cert", userId: "u_1", read: false, createdAt: new Date() },
  ];

  const mockNotifRepo = {
    createNotification: async (n: any) => ({ id: "n_new", ...n, createdAt: new Date(), updatedAt: new Date() }),
    findByUserId: async (uid: string) => store.filter((n) => n.userId === uid || n.userId === null) as any,
    ...
    getUnreadCount: async (uid: string) =>
      store.filter((n) => (n.userId === uid || n.userId === null) && !n.read).length,
  };
...
test("Notification Security: Student A cannot view or tamper with Student B's notifications", async () => {
  const store = [
    { id: "n_a1", title: "Payment A", message: "Course A unlocked", userId: "student_a", read: false, createdAt: new Date() },
    { id: "n_a2", title: "Certificate A", message: "Cert ready", userId: "student_a", read: false, createdAt: new Date() },
    { id: "n_b1", title: "Payment B", message: "Course B unlocked", userId: "student_b", read: false, createdAt: new Date() },
  ];
  ...
  const mockNotifRepo = {
    ...
    findByUserId: async (uid: string) => store.filter((n) => n.userId === uid || n.userId === null) as any,
    markAsRead: async (id: string, uid?: string) => {
      const item = store.find((n) => n.id === id && (!uid || n.userId === uid || n.userId === null));
      if (item) {
        item.read = true;
        return true;
      }
  ...
  assert.ok(studentANotifs.notifications.every((n) => n.userId === "student_a"));
  assert.equal(studentANotifs.notifications.some((n) => n.id === "n_b1"), false);
...
test("Dashboard Algorithm: verifies Resume Course priority and 0-division safety", () => {
  ...
  const courses = [
    { id: "c1", title: "React Mastery", totalLessons: 10, completedLessons: 10, progress: 100, lastUpdated: 1000 },
    { id: "c2", title: "Node Architecture", totalLessons: 10, completedLessons: 4, progress: 40, lastUpdated: 2000 },
    { id: "c3", title: "TypeScript Basics", totalLessons: 10, completedLessons: 2, progress: 20, lastUpdated: 500 },
  ];
  ...
  const safeProgress = zeroLessonCourse.totalLessons > 0
    ? Math.round((zeroLessonCourse.completedLessons / zeroLessonCourse.totalLessons) * 100)
    : 0;
